...
class Country:
    def __init__(self, name: str, rating: str):
        if rating not in RATING_TO_PD:
            raise ValueError(f"Unknown rating '{rating}'. Valid ratings: {list(RATING_TO_PD)}")
        self.name = name
        self.rating = rating
...
class Policy:
    ...
    def duration_years(self) -> float:
        return (self.expiry_date - self.effective_date).days / 365.25

    def max_lol(self) -> float:
        return max(lol for _, lol in self.exposures)

    def calculate_pd(self) -> float:
        raise NotImplementedError("Subclasses must implement calculate_pd()")
...
class SingleCountryPolicy(Policy):
    def calculate_pd(self) -> float:
        country, _ = self.exposures[0]
        return country.pd


class MultiCountryPolicy(Policy):
    def calculate_pd(self) -> float:
        tenor = min(max(round(self.duration_years), 1), 3)
        b = B_FACTOR[tenor]
